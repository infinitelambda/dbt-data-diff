...
async function getDataBatch(page) {
    const response = await fetch(`https://api.github.com/search/repositories?q=topic:dbt-data-diff&per_page=100&page=${page}`, { headers: { Accept: 'application/vnd.github.mercy-preview+json' } })
...
async function main() {
    if (div) {
        ...
        data.forEach(v => {
            if (v.full_name === 'infinitelambda/dbt-data-diff') {
                return
            }
